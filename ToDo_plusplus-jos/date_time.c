...
int isPastDeadline(const char *deadline)
{
  int d, m, y, H, M;
  if (sscanf(deadline, "%d.%d.%d %d:%d", &d, &m, &y, &H, &M) == 5 ||
    sscanf(deadline, "%d-%d-%d %d:%d", &d, &m, &y, &H, &M) == 5) {
    }
  //ohne Uhrzeit
  else if (sscanf(deadline, "%d.%d.%d", &d, &m, &y) == 3 ||
           sscanf(deadline, "%d-%d-%d", &d, &m, &y) == 3) {
    H = 0;
    M = 0;
           } else {
